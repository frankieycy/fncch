...
class FunctionalNetwork:
    ...
    def reconstruct(self, w):
        ...
        * threshold criteria imposed (exc nodes: 2 sd, inh nodes: 1 sd)
        ...
        self.log = {
            't_exc'     : t_exc     if i_exc.size>0 else [],
            'C_exc'     : C_exc     if i_exc.size>0 else [],
            'idx_exc'   : idx_exc   if i_exc.size>0 else [],
            't_inh'     : t_inh     if i_inh.size>0 else [],
            'C_inh'     : C_inh     if i_inh.size>0 else [],
            'idx_inh'   : idx_inh   if i_inh.size>0 else [],
